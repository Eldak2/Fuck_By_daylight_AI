...
class LamaChatWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        from src.lama.lama_knowledge import LamaKnowledge
        from src.lama.lama_searcher import LamaSearcher
        self.knowledge = LamaKnowledge()
        self.searcher = LamaSearcher(self.knowledge)

        # Запускаем обновление знаний в фоне
        self.update_timer = QTimer()
        self.update_timer.timeout.connect(self.update_knowledge)
        self.update_timer.start(10000)  # каждые 10 секунд
    ...
    def update_knowledge(self):
        game = self.searcher.update_knowledge()
        if game:
            # Выводим краткую информацию в чат (если это первое обнаружение)
            summary = self.knowledge.get_game_summary(game)
            if summary:
                self.add_message('bot', f"📚 Обновлена информация об игре: {summary}")
    ...
    def send_message(self):
        ...
        def do_send():
            try:
                ...
                game = self.searcher.detect_game()
                if game:
                    facts = self.knowledge.get_all_facts(game)
                    if facts:
                        # Отвечаем из базы знаний, если есть подходящая информация
                        reply = f"Я знаю об игре {game}:\n"
                ...
                # Если ничего нет в базе, отправляем запрос к основному агенту
